...
<s:iterator value="#page.list" var="obj">
        	<!--item-->
        <div class="item" >
            <div><a href="${ pageContext.request.contextPath }/noticeAction_getNotice?notice_id=${ obj.notice_id }">${ obj.notice_title }</a></div>
...
<s:if test="#page.currentPage < #page.totalPage">
        ...
        <div id="loadMore" style="margin: 10px; border-radius: 5px;background-color: #F9F9F9; height: 10px; line-height: 10px;text-align: center;">
            <a href="javascript:void(0);" style="color: #624645;" onclick="loadMoreNotice(${ page.currentPage + 1 })">点击加载更多</a>
        </div><!--加载更多-->
</s:if>
    </div><!--左侧-->
...
<s:iterator value="#hotNotices" var="hotNotice" status="status">
        <!--通知条目-->
        <div style="margin: 20px 0px;">
            ...
            <span>
                <a href="${ pageContext.request.contextPath }/noticeAction_getNotice?notice_id=${ hotNotice.notice_id }">${ hotNotice.notice_title }</a>
            </span>
            <span style="color: #75B0B0;">${ hotNotice.student.stu_name  }</span>
            <span style="color: #D8B092;">${ hotNotice.notice_date }</span>
        </div>
        <hr>
        <!--通知条目END-->
</s:iterator>
...
<script>
    ...
    function loadMoreNotice(page) {
    	$.ajax({
    		url:"${ pageContext.request.contextPath }/noticeAction_getNotices",
    		data: {currentPage:page},
    		type: "get",
    		success: function(page){
    			for (var i=0; i<page.list.length; i++){
    				var newHtml = "<div class=\"item\" >\n" +
    		        "            <div><a href=\"${ pageContext.request.contextPath}/noticeAction_getNotice?notice_id="+page.list[i].notice_id+"\">"+ page.list[i].notice_title +"</a></div>\n" +
    		        "            <div class=\"item-author\">\n" +
    		        "                <i class=\"fa fa-user-o\" aria-hidden=\"true\"></i>\n" +
    		        "                "+ page.list[i].student.stu_name +"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n" +
    		        "                <i class=\"fa fa-calendar\" aria-hidden=\"true\"></i>\n" +
    		        "                "+page.list[i].notice_date +"\n" +
    		        "                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n" +
    		        "                <i class=\"fa fa-eye\" aria-hidden=\"true\"></i>\n" +
    		        "                阅读("+page.list[i].notice_scan +")\n" +
    		        "            </div>\n" +
    		        "            <div class=\"item-abstract\">\n" +
    		        "               "+page.list[i].notice_abstract +"\n" +
    		        "            </div>\n" +
    		        "        </div><!--item-->";
    			...
    			if (page.currentPage<page.totalPage) {
    				...
    				$($a).attr("onclick","loadMoreNotice("+pageNo+")");
    			} else {
